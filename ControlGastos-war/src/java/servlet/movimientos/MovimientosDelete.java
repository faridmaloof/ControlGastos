 ...
 * @author farid
 */
@WebServlet(name = "MovimientosDelete", urlPatterns = {"/MovimientosDelete"})
public class MovimientosDelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (movimiento.delete(element.getId()))
            request.setAttribute("success", "El registro se ha eliminado con exito.");
        else
            request.setAttribute("danger", "El registro no pudo ser eliminado.");
        /*if (helpers.Connections.insertarUpdate("UPDATE categorias SET activo = 1 WHERE id = "+request.getParameter("element"))>0)
